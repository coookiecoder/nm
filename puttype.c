...
/*   Updated: 2024/03/02 12:50:39 by abareux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	puttype_32(Elf32_Sym symbol, Elf32_Ehdr file_eheader, void *file_data)
{
//	unsigned char	type = ELF32_ST_TYPE(symbol.st_info);
	unsigned char	bind = ELF32_ST_BIND(symbol.st_info);
//	unsigned char	info = ELF32_ST_INFO(bind, type);
	Elf32_Shdr		*linked_section_header = file_data + file_eheader.e_shoff + file_eheader.e_shentsize * symbol.st_shndx;
